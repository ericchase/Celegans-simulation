...
#define RADIANS_TO_DEGREE 57.2957f
...
LOG (void Celegans_free (Celegans * ce){)
    Celegans_brain_free(ce->soul);
    free(ce);
}

LOG (void Celegans_update (Celegans * ce){)
    ...
    {
        // NOTE: downcasting float to int
        rightAcc += (int) ce->soul->neurons[i]->state;
        ce->soul->neurons[i]->state *= 0.7f; //Decrease the muscle contraction over time 
    }
    ...
    {
        // NOTE: downcasting float to int
        leftAcc += (int) ce->soul->neurons[i]->state;
        ce->soul->neurons[i]->state *= 0.7f; //Decrease the muscle contraction over time
    }

    //Calculate speed based on muscle contraction
    float speed = (float) (leftAcc + rightAcc);
    speed = speed / 200.0f * 3.0f;
    ...
    float changex = cosf(ce->body.rotation / RADIANS_TO_DEGREE) * speed;
    float changey = sinf(ce->body.rotation / RADIANS_TO_DEGREE) * speed;
...
LOG (void Celegans_draw (Celegans * ce){)
...
LOG (int main (int argc, char * argv[]){)
    ...
    for (;;)
    {
        ...
        int collision = 0;
        if (ce->body.position.x <= 0)
        {
            ce->body.position.x = 0;
            collision = 1;
        }
        if (ce->body.position.x >= WINDOW_WIDTH - BODY_HEIGHT)
        {
            ce->body.position.x = WINDOW_WIDTH - BODY_HEIGHT;
            collision = 1;
        }
        if (ce->body.position.y <= 0)
        {
            ce->body.position.y = 0;
            collision = 1;
        }
        if (ce->body.position.y >= WINDOW_HEIGHT - BODY_HEIGHT)
        {
            ce->body.position.y = WINDOW_HEIGHT - BODY_HEIGHT;
            collision = 1;
        }
        ...
        if (drawbraindata)
        {
            ...
            {
                int x = i % 32;
                int y = (int) ((double) i / (double) 32);
                if (Celegans_brain_fired(ce->soul, i))
                    vsRendererDrawRect(NULL, 0, 0, 0, 0, (float) (x * 16 + 1), (float) (y * 16 + 1), 16, 16, VS_COLOR_WHITE);
            }
            ...
            for (int i = MUSCLE_RIGHT_START; i < MUSCLE_RIGHT_END; i++)
            {
                ...
                int y = 200;
                if (ce->soul->neurons[i]->state > 0) vsRendererDrawRect(NULL, 0, 0, 0, 0, (float) (x * 8 + 1), (float) y, 8, 8, VSCOLOR(255, 0, 0, (unsigned char) (ce->soul->neurons[i]->state * 8.2)));
                else vsRendererDrawRect(NULL, 0, 0, 0, 0, (float) (x * 8 + 1), (float) y, 8, 8, VSCOLOR(0, 0, 255, (unsigned char) (ce->soul->neurons[i]->state * -8.2)));
            }
