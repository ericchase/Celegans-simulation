...
#define VS_TEXT_ALIGN_CENTER 1
    ...
    ///
    ////////////////////////////////////////////////////////////
    typedef struct vsVector2
    {
        vsVector2 ()
            : x(0),
            y(0) {}

        vsVector2 (float x, float y)
            : x(x),
            y(y) {}
...
#define VSVECTOR2(x, y) vsVector2(x, y)
    ...
    typedef struct vsRect
    {
        ...
        vsRect (float x, float y, float w, float h)
            : x(x),
            y(y),
            w(w),
            h(h) {}
...
#define VSRECT(x, y, w, h) vsRect(x, y, w, h)
    ...
    typedef struct vsColor
    {
        ...
        vsColor (unsigned char r, unsigned char g, unsigned char b, unsigned char a)
            : r(r),
            g(g),
            b(b),
            a(a) {}
...
////////////////////////////////////////////////////////////
LOG (void vsGLStateBindTexture (unsigned int id){)
...
////////////////////////////////////////////////////////////
LOG (void vsGLStateBindShader (unsigned int id){)
...
////////////////////////////////////////////////////////////
LOG (void vsGLStateBindBuffer (unsigned int id, unsigned int type){)
...
////////////////////////////////////////////////////////////
LOG (void vsViewSet (vsView view){)
    vsVector2 NCenter;
    NCenter.x = (float) floor(view.center.x);
    NCenter.y = (float) floor(view.center.y);
    ...
    //Set viewport
    glViewport(
        (GLsizei) view.viewport.x,
        (GLsizei) view.viewport.y,
        (GLsizei) view.viewport.w,
        (GLsizei) view.viewport.h
    );
...
////////////////////////////////////////////////////////////
LOG (void vsViewCreate (vsRect viewport, vsVector2 center, vsVector2 size, float angle){)
    vsView view;
...
////////////////////////////////////////////////////////////
LOG (void vsBitmapDestroy (vsBitmap * bitmap){)
    if (bitmap->data)
        free(bitmap->data);
    free(bitmap);
}
...
////////////////////////////////////////////////////////////
LOG (void vsBitmapUnload (vsBitmap * bitmap){)
    if (bitmap->data != NULL)
        free(bitmap->data);
    bitmap->data = NULL;
    bitmap->width = 0;
...
////////////////////////////////////////////////////////////
LOG (void vsBitmapLoad (vsBitmap * bitmap, const char * filepath){)
    //Unload bitmap
...
////////////////////////////////////////////////////////////
LOG (void vsBitmapLoadFromData (vsBitmap * bitmap, unsigned char * data, int width, int height, int channels){)
    bitmap->data = data;
    bitmap->width = width;
...
////////////////////////////////////////////////////////////
LOG (vsVector2 vsBitmapGetSize (vsBitmap * bitmap){)
    return VSVECTOR2((float)bitmap->width, (float)bitmap->height);
}

////////////////////////////////////////////////////////////
LOG (int vsBitmapGetChannels (vsBitmap * bitmap){)
    return bitmap->channels;
}

////////////////////////////////////////////////////////////
LOG (const unsigned char * vsBitmapGetData (vsBitmap * bitmap){)
    return bitmap->data;
}
...
////////////////////////////////////////////////////////////
LOG (void vsTextureDestroy (vsTexture * texture){)
    if (texture) free(texture);
}

////////////////////////////////////////////////////////////
LOG (void vsTextureUnload (vsTexture * texture){)
    if (texture->id)
...
///////////////////////////////////////////////////////////
LOG (void vsTextureLoadFromData (vsTexture * texture, vsBitmap * bitmap){)
    if (!texture) return;
    ...
    switch (bitmap->channels)
    {
        ...
        case 4: fmt = GL_RGBA;
            break;
        default: printf("Error in vsTexture : data file format [%d] not recognized\n", fmt);
            return;
    }
    ...
    texture->size.x = (float) bitmap->width;
    texture->size.y = (float) bitmap->height;
    texture->smooth = VS_TRUE;
    texture->repeated = VS_FALSE;
}

////////////////////////////////////////////////////////////
LOG (void vsTextureLoad (vsTexture * texture, const char * filepath){)
    //Load bitmap
...
////////////////////////////////////////////////////////////
LOG (void vsTextureSetSmooth (vsTexture * texture, const int smooth){)
    if (!texture) return;
...
////////////////////////////////////////////////////////////
LOG (void vsTextureSetRepeated (vsTexture * texture, const int repeated){)
    if (!texture) return;
...
////////////////////////////////////////////////////////////
LOG (vsVector2 vsTextureGetSize (vsTexture * texture){)
    if (!texture) return VSVECTOR2(0, 0);
    return texture->size;
}

////////////////////////////////////////////////////////////
LOG (int vsTextureIsSmooth (vsTexture * texture){)
    if (!texture) return -1;
    return texture->smooth;
}

////////////////////////////////////////////////////////////
LOG (int vsTextureIsRepeated (vsTexture * texture){)
    if (!texture) return -1;
...
////////////////////////////////////////////////////////////
LOG (void vsShaderDestroy (vsShader * shader){)
...
////////////////////////////////////////////////////////////
LOG (void vsShaderParamCacheResize (vsShader * shader, int size){)
...
////////////////////////////////////////////////////////////
LOG (void vsShaderParamCachePush (vsShader * shader, const char * name, int location){)
...
////////////////////////////////////////////////////////////
LOG (void vsShaderUnload (vsShader * shader){)
    if (shader->id)
    {
...
////////////////////////////////////////////////////////////
LOG (int vsShaderCompileShader (const char * source, int shadertype){)
    unsigned int sp = 0;
...
////////////////////////////////////////////////////////////
LOG (void vsShaderCompile (vsShader * shader, const char * source, int shadertype){)
    //Unload shader
...
////////////////////////////////////////////////////////////
LOG (void vsShaderCompile2 (vsShader * shader, const char * vertexsource, const char * fragmentsource){)
    //Unload shader
    vsShaderUnload(shader);
...
////////////////////////////////////////////////////////////
LOG (void vsShaderLoad (vsShader * shader, const char * filepath, int shadertype){)
    //Variables
    int fsize = 0;
...
////////////////////////////////////////////////////////////
LOG (void vsShaderLoad2 (vsShader * shader, const char * vertexshader_path, const char * fragmentshader_path){)
    //Variables
    int fsize = 0;
    ...
    //Open the shader file
    FILE * vertexsh_file;
    fopen_s(&vertexsh_file, vertexshader_path, "rb");
    ...
    //Open the shader file
    FILE * fragmentsh_file;
    fopen_s(&fragmentsh_file, fragmentshader_path, "rb");
...
///////////////////////////////////////////////////////////
LOG (void vsShaderBind (vsShader * shader){)
    vsGLStateBindShader(shader->id);
}

////////////////////////////////////////////////////////////
LOG (unsigned int vsShaderGetID (vsShader * shader){)
    return shader->id;
}

////////////////////////////////////////////////////////////
LOG (int vsShaderGetParamLocation (vsShader * shader, int paramtype, const char * paramname){)
    //Search in the cache
    int i;
...
////////////////////////////////////////////////////////////
LOG (void vsShaderSetParam1 (vsShader * shader, const char * paramname, int paramtype, const float x){)
    //Check for errors
    if (!shader->id)
    {
...
////////////////////////////////////////////////////////////
LOG (void vsShaderSetParam2 (vsShader * shader, const char * paramname, int paramtype, const float x, const float y){)
    //Check for errors
    if (!shader->id)
    {
        printf("Error in vsShader : can't set parameter to an empty shader\n");
...
////////////////////////////////////////////////////////////
LOG (void vsShaderSetParam3 (vsShader * shader, const char * paramname, int paramtype, const float x, const float y, const float z){)
    //Check for errors
    if (!shader->id)
    {
        printf("Error in vsShader : can't set parameter to an empty shader\n");
...
////////////////////////////////////////////////////////////
LOG (void vsShaderSetParam4 (vsShader * shader, const char * paramname, int paramtype, const float x, const float y, const float z, const float w){)
    //Check for errors
    if (!shader->id)
    {
        printf("Error in vsShader : can't set parameter to an empty shader\n");
...
////////////////////////////////////////////////////////////
LOG (void vsRendererClear (){)
    glClear(GL_COLOR_BUFFER_BIT);
}

////////////////////////////////////////////////////////////
LOG (void vsRendererDisplay (){)
...
////////////////////////////////////////////////////////////
LOG (void vsRendererDraw (vsTexture * texture, float x, float y, float x2, float y2, float x3, float y3, float x4, float y4, float tx_x, float tx_y, float tx_w, float tx_h, vsColor color){)
    if (texture == NULL)
    {
...
////////////////////////////////////////////////////////////
LOG (void vsRendererDrawSprite (vsSprite * sprite){)
    static vsVector2 v1;
    static vsVector2 v2;
    static vsVector2 v3;
    static vsVector2 v4;
    ...
    if (sprite->rotation)
    {
        const float ca = cosf(sprite->rotation * 3.14159f / 180);
        const float sa = sinf(sprite->rotation * 3.14159f / 180);
...
////////////////////////////////////////////////////////////
LOG (void vsRendererDrawRect (vsTexture * texture, float tx, float ty, float tw, float th, float x, float y, float w, float h, vsColor color){)
    if (texture == NULL)
    {
...
////////////////////////////////////////////////////////////
LOG (void vsRendererToggleWireFrame (){)
