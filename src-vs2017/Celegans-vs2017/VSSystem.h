...
////////////////////////////////////////////////////////////
LOG (int vsSystemInit (const char* title, int w, int h, int fps, int style){)
	//Initialize SDL
	...
	winheight = h;
	winopen = VS_TRUE;
	window = SDL_CreateWindow(title, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, w, h, winstyle);
    LOGD(winstyle);
    LOGP(window);
    ...
    SDL_GL_SetAttribute( SDL_GL_CONTEXT_PROFILE_MASK, SDL_GL_CONTEXT_PROFILE_CORE ); /// NEW
	glcontext = SDL_GL_CreateContext(window);
    LOGP(glcontext);
    LOGS(SDL_GetError());

    LOGD(fps)
	int ret = SDL_GL_SetSwapInterval(fps);
    LOGD(ret)
    LOGS(SDL_GetError());
...
////////////////////////////////////////////////////////////
void vsTimeSleep(Uint32 millisecs)
{
	SDL_Delay(millisecs);
}
